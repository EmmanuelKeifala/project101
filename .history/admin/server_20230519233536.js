...
app.get("/death-info/:id", async (req, res, next) => {
	...
	app.put("/api/applications/:id", async (req, res) => {
		...
		try {
			...
			const application = await listingsCollection.findOne({ _id: id });

			if (!application) {
				throw new Error("Application not found");
			}

			// Delete the application from the Listings collection
			await listingsCollection.deleteOne({ _id: id });

			// Move the application to the AcceptedApplications collection
			await acceptedCollection.insertOne(application);
			...
			try {
				await sendMail({
					...
					message: `Hello ${application.name}, We are very happy to inform you that your application for a birth certificate have been vetted and it has been accepted`,
				});
				res.status(201).json({
					success: true,
					message: `email sent!`,
