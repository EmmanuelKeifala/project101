...
import BirthPreview from "./scenes/BirthPreview";
function App() {
	const setDataToStore = useApplicationsStore((state) => state.setApplications);
	const setAcceptedApplicationToStore = useAcceptedApplicationInfoStore(
		(state) => state.setAcceptedApplication,
	);
	const setDeclinedApplicationToStore = useDeclinedApplicationInfoStore(
		(state) => state.setDeclinedApplication,
	);
	...
	useEffect(() => {
		...
		const acceptedApplication = () => {
			fetch("http://localhost:3001/api/accepted")
				.then((response) => {
					if (!response.ok) {
						throw new Error("Error retrieving listings");
					}
				...
				.then((data) => {
					setAcceptedApplicationToStore(data);
				})
				.catch((error) => {
					console.error("Error:", error.message);
				});
		...
		const declinedApplication = () => {
			fetch("http://localhost:3001/api/declined")
				.then((response) => {
					if (!response.ok) {
						throw new Error("Error retrieving listings");
					}
				...
				.then((data) => {
					setDeclinedApplicationToStore(data);
				})
				.catch((error) => {
					console.error("Error:", error.message);
				});
						...
						<div className="app">
							...
							<main className="content">
								...
								<Routes>
									...
									<Route path="/birth-info/:id" element={<BirthPreview />} />
